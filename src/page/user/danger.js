...
export default function Danger() {
  const [selected, setSelected] = useState(danger[0]);
  const [customDanger, setCustomDanger] = useState("");

  const handleSelectedChange = (value) => {
    setSelected(value);
    if (value.name === "[기타(직접입력)]") {
      setCustomDanger(""); // Reset the custom danger input field
    }
  };
  ...
  return (
    <div id="danger" className="flex items-baseline justify-start">
      ...
      {selected.name === "[기타(직접입력)]" && (
        <input
          type="text"
          value={customDanger}
          onChange={(e) => setCustomDanger(e.target.value)}
