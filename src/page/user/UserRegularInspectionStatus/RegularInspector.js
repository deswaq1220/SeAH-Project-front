...
const status = [
  { id: 0, name: "완료", value: "OK" },
  { id: 1, name: "미완료", value: "NO" },
];
// 점검자
export default function RegularInspector({ onFormDataChange }) {
  // 점검자정보
  ...
  const [selected, setSelected] = useState({ name: "선택", value: null });
  // 입력란 변경 시 상태 업데이트 함수
  ...
  const handleSelected = (selectedData) => {
    ...
    onFormDataChange({
      ...
      speComplete:selectedData.value === '00' ? null : selectedData.value,
    });
};
