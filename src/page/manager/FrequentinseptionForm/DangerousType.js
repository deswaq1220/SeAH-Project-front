...
export default function DangerousType({onFormDataChange}) {
  const { masterdataPart, masterdataId } = useParams();
  const [specialDangerList, setSpecialDangerList] = useState([]);
  const [selectedDanger, setSelectedDanger] = useState("");  // 선택위험분류
  ...
  // 위험분류 선택 시 selectedPart 값 업데이트하고 onFormDataChange 호출
  const handleSelectedDanger = (value) => {
    ...
    <>
      <div className="flex flex-col ">
        <div className="flex items-center">
          ...
          <Listbox value={selectedDanger} onChange={handleSelectedDanger}>
            {({ open }) => (
              <>
                <div className="relative mt-2">
                  ...
                  <span className="block truncate">{selectedDanger ? selectedDanger.dangerMenu : ''}</span>
